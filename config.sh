# windows share mount ... should match with /etc/fstab
# mdbf2="/mnt/fingerprints/Program\ Files/ZKTeco/Access.mdb"
mdbf2="/mnt/minerva-laptop/ZKTeco/ZKAccess3.5/Access.mdb"
fpetc="/home/minerva" # workspace folder
mdbf="${fpetc}/Access.mdb"
df="${fpetc}/fingerprints-adapter-lastupdate.txt"
lockfile="${fpetc}/fingerprints-lock.txt"

instdir="/home/shadi/Development/ffa-zkteco-mfbf"

mysqlUser=ffaFingerprints
mysqlPass=fp1298

